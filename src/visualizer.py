...
import cv2
from mediapipe.python.solutions.drawing_utils import (DrawingSpec, draw_detection, draw_landmarks,)
...
class Visualizer:
    ...
    def face_area_window(
        face_area: List[dict], video: Video, frame, results=None, compatible_ids: list = None,
    ):
        frame_width = video.cap_width
        frame_height = video.cap_height
    ...
    def head_pose_plotter(frame: ndarray, head_pose: dict):
        ...
        cv2.putText(
            frame, f"depth: {origin[2]}", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 0), 2,
        )
