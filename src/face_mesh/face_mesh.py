...
class HeadPoseEstimation():
    ...
    def __call__(self, input_v: str, areas: List[dict], output: str) -> List[str]:
    ...
    def apply_face_mesh(
        self, q_out: Queue, input_v: str, area: dict, area_id: int, output: str, progress: bool
    ) -> List[str]:
    ...
    def write_result(self, results: np.ndarray, term: Iterable[int], area_id: int) -> str:
    ...
    def create_args(self, step: int, areas: List[dict], frame: np.ndarray, recognizers: list,):
        args = []
    ...
    def create_dict(
        self, step: int, area_info: dict, origin: Iterable, angle: Iterable, landmarks: Iterable
    ) -> dict:
    ...
    def trim_area(self, frame: np.ndarray, area: dict) -> Iterable[int]:
        ...
        return int(x_lw * frame_w), int(x_up * frame_w), int(y_lw * frame_h), int(y_up * frame_h)
    
    def process(self, step: int, area: dict, frame: np.ndarray, face_mesh: FaceMesh) -> List[dict]:
        ...
        if recognission.multi_face_landmarks:
            for face_landmarks in recognission.multi_face_landmarks:
                ...
                head_pose = self.create_dict(step, area, nose_3d, angles, face_landmarks)
    ...
    @staticmethod
    def local2global(area: dict, frame_size: Iterable[int],
                        coordinate: Iterable[float]) -> Tuple[float]:
        ...
        if len(coordinate) == 3:
            ...
            return (
                absolute_x / frame_size[0], absolute_y / frame_size[1], absolute_z / frame_size[0]
            )
